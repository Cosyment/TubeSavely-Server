...
class DouyinIE(TikTokBaseIE):
    ...
    def generate_request_params(self, url, user_agent):
        query = urllib.parse.urlparse(url).query
        print(f'query: {query}')

        project_path = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        file_path = os.path.join(project_path, 'utils', 'douyin_x-bogus.js')
        # file_path = os.path.join(os.getcwd(), 'vendor/yt_dlp/utils', 'douyin_x-bogus.js')
        xbogus = execjs.compile(open(file_path).read()).call('sign', query, user_agent)
    ...
    def get_ttwid(self):
        ...
        ttwid = None
        url = 'https://v.douyin.com/iFQ87sqD/'
    ...
    def _real_extract(self, url):
        ...
        agent = UserAgent().chrome
        msToken = self.generate_random_str()
        url_with_bogus = self.generate_request_params(
            url=f'https://www.douyin.com/aweme/v1/web/aweme/detail/?aweme_id={video_id}&aid=1128&version_name=23.5.0&device_platform=android&os_version=2333',
            user_agent=agent)["param"]
        ttwid_ = self.get_ttwid()
        if ttwid_:
            ttwid = ttwid_
        else:
        ...
        res = {}
        proxies = {
            # 'http': 'http://141.148.147.47.:9527',  # HTTP代理IP
            'http': 'https://175.178.252.202:8080'  # HTTPS代理IP
        }

        for i in range(5):
            data = requests.get(url=url_with_bogus, headers=headers, timeout=15)
            if str(data.text).__contains__('aweme_detail'):
                res = data.json()['aweme_detail']
                break
            time.sleep(3)
        return self._parse_aweme_video_app(res)
