...
class WeiBo(BaseParser):
    ...
    def parse_share_url(self, share_url: str) -> VideoInfo:
        if "show?fid=" in share_url:
            # video_id = self.get_val_from_url_by_query_key(share_url, "fid")
            video_id = vendor.yt_dlp.extractor.extend.get_val_from_url_by_query_key(share_url, "fid")
        else:
    ...
    def parse_video_id(self, video_id: str) -> VideoInfo:
        ...
        response = requests.post(req_url, headers=headers, data=post_content, allow_redirects=True)
        response.raise_for_status()
